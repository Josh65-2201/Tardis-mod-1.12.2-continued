...
public class MessageChangeRoom implements IMessage{

	int index = -1;
	BlockPos pos = BlockPos.ORIGIN;
	
	public MessageChangeRoom() {}
	
	public MessageChangeRoom(int id, BlockPos pos) {
		this.index = id;
		this.pos = pos;
	}
	...
	public static class Handler implements IMessageHandler<MessageChangeRoom, IMessage>{

		@Override
		public IMessage onMessage(MessageChangeRoom message, MessageContext ctx) {
			ctx.getServerHandler().player.getServerWorld().addScheduledTask(new Runnable(){
				@Override
				public void run() {
					if(!Room.ROOMS.isEmpty() && Room.ROOMS.size() > message.index) {
						...
						Room.ROOMS.get(message.index).generate(ctx.getServerHandler().player.getServerWorld(), message.pos);
