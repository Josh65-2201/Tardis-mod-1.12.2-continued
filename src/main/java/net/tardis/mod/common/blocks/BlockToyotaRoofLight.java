...
public class BlockToyotaRoofLight extends Block implements IARSBlock {
    private final boolean isOn;

    public BlockToyotaRoofLight(boolean isOn) {
    ...
    public void onBlockAdded(World worldIn, BlockPos pos, IBlockState state) {
        if (!worldIn.isRemote) {
            if (this.isOn && !worldIn.isBlockPowered(pos)) {
                worldIn.setBlockState(pos, TBlocks.toyota_roof_light_off.getDefaultState(), 2);
            } else if (!this.isOn && worldIn.isBlockPowered(pos)) {
                worldIn.setBlockState(pos, TBlocks.toyota_roof_light_on.getDefaultState(), 2);
            }
        }
    }
